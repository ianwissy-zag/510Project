...
module crossbar_mac #(
    ...
    // 4 output column accumulations (signed)
    ...
    // ==============================================================================
    // 2. Parallel Accumulation Stage (Column summation)
    // ==============================================================================
    genvar c;
    generate
        for (c = 0; c < 4; c++) begin : gen_accum
            always_comb begin
                // Summing all multiplied results down the vertical column
                mac_outs_o[c] = mult_results[0][c] + 
                                mult_results[1][c] + 
                                mult_results[2][c] + 
                                mult_results[3][c];
            end
