...
//
// The gelu_unit has a 9-cycle latency (8 register banks + 1 input pre-register
// in axi_readback).  When GELU is active, a warmup counter runs for
// GELU_LATENCY=9 cycles before the AXI output begins, pre-filling the pipeline
// so that by the time the first beat is transmitted the result is correctly
// aligned with M row 0.
//
// sram_addr = total_cnt >> 1 (two AXI beats per M row).
// At output cycle total_cnt T (T >= GELU_LATENCY), the GELU result was
// produced from sram_addr (T - GELU_LATENCY) >> 1 = M row (out_cnt >> 1).
//
// SRAM address sequence (GELU_LATENCY=9 example):
//   warmup: total_cnt 0..8  → addr = 0,0,1,1,2,2,3,3,4  (tvalid=0)
//   output: total_cnt 9,10  → addr = 4,5  (GELU result = M row 0) ✓
//           total_cnt 11,12 → addr = 5,6  (GELU result = M row 1) ✓
//           ...
//
...
module axi_readback #(
    ...
    localparam VALS_PER_BEAT = AXI_WIDTH / PSUM_WIDTH;  // 8
    ...
    localparam CNT_W = M_ADDR_W + 4;  // sufficient for 9 + 2*256 = 521
