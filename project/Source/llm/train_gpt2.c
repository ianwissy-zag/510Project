...
int main() {
...
#ifdef SINGLE_PASS
    int max_steps = 1;
    printf("[cpu] SINGLE_PASS enabled — running 1 training step only.\n");
#else
    int max_steps = 40;
#endif
    ...
    // dump weight parameters for comparison with accelerated implementation
    {
        const char* dump_path = "weights_cpu.bin";
        FILE* f = fopen(dump_path, "wb");
        if (f) {
            fwrite(model.params_memory, sizeof(float), model.num_parameters, f);
            fclose(f);
            printf("[cpu] weights dumped to %s (%zu parameters)\n",
