...
//
// Tests 1-4: inherited streaming correctness (same as HDL_Systolic_16x32)
...
int main() {
    ...
    // ── Test 5: bias addition — W=1, act=1, bias=0.5 → C=ROWS+0.5 ───────────
    {
        ...
        float expected = (float)ROWS + 0.5f;  // 32.0 + 0.5 = 32.5
    ...
    // ── Test 6: GELU only — W=1, act=1, no bias → GELU(ROWS) ────────────────
    {
        ...
        float expected = gelu_ref((float)ROWS);  // GELU(32) ≈ 32.0
    ...
    // ── Test 7: bias + GELU — W=1, act=1, bias=0.5 → GELU(ROWS+0.5) ─────────
    {
        ...
        float expected = gelu_ref((float)ROWS + 0.5f);  // GELU(32.5) ≈ 32.5
