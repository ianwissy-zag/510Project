// End-to-end BF16 testbench for vec_mac_top — 512-wide vector MAC.
//
...
//   psum[j] = 32 × 1.0 × 1.0 = FP32(32.0) = 0x42000000  (all 512 lanes)
//
...
//
// AXI protocol:
//   Weight: 16 × 512-bit beats per SRAM row (32 BF16 elements per beat)
//           32 rows → 512 beats total per weight matrix
//   Act:    1 × 512-bit beat per ping/pong slot (32 BF16 elements)
//   Readback: 32 × 512-bit beats (512 × FP32 = 16384 bits)
...
static const int VEC  = 512; // VEC_SIZE
static const int K    = 32;  // K_DEPTH
static const int WRD  = 16;  // AXI words per beat (512 / 32)
static const int BEATS_PER_WT_ROW = 16; // VEC*WT_WIDTH/AXI = 512*16/512
static const int BEATS_READBACK   = 32; // VEC*PSUM_WIDTH/AXI = 512*32/512
...
static void tick(Vvec_mac_top* dut) {
    dut->clk = 0; dut->eval();
    if (g_trace) g_trace->dump(g_tick_count * 10);
    dut->clk = 1; dut->eval();
    if (g_trace) g_trace->dump(g_tick_count * 10 + 5);
    dut->clk = 0; dut->eval();
    ++g_tick_count;
}
...
int main(int argc, char** argv) {
    ...
    g_trace->open("../sim/compute_core.vcd");
