`timescale 1ns / 1ps
// Top-level 512-wide BF16 vector MAC accelerator.
//
// Architecture:
//   - 512 parallel MAC units, each computing psum[j] += act[k] * W[k][j].
//   - BF16 inputs (custom 8×8-bit multiplier) with FP32 accumulators.
//   - No separate LOAD_WT phase: weight rows stream from weight_sram
//     directly into all 512 MAC units each cycle during COMPUTE.
//   - Ping-pong weight and activation SRAMs for double-buffering.
//   - Each tile: K_DEPTH=32 cycles of BF16 multiply, FP32 accumulate.
//   - Weight SRAM row width: 8192 bits (512 × 16-bit BF16 elements).
//   - Activation SRAM row width: 512 bits (32 × 16-bit BF16 elements).
//   - Output SRAM row width: 16384 bits (512 × 32-bit FP32 psums).
//   - AXI readback: 32 × 512-bit beats per output row.
//
// FPU: custom BF16×BF16 → FP32 (bf16_mul + bf16_fp32_add)
//
...
//   Weight write: 16 beats per row × K_DEPTH rows = 512 beats per matrix
//   Act write:    1 beat per ping/pong slot (full K-vector packed)
//   Readback:     32 beats (16384-bit output in 512-bit chunks)
module vec_mac_top #(
